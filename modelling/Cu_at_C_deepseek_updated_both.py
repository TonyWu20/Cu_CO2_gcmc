...
def write_lammps_data(filename, atoms, atom_types_dict, box_size, description=""):
    """Write LAMMPS data file in atom_style full format"""
    ...
    with open(filename, "w") as f:
        ...
        chunk_size = 10000
        for i in tqdm(
            range(0, len(atoms), chunk_size), desc=f"Writing atoms to {filename}"
        ):
            chunk_end = min(i + chunk_size, len(atoms))
            chunk_lines = []
            ...
            for j in range(i, chunk_end):
                ...
                centered_pos = pos + box_size / 2 - np.mean(positions, axis=0)
                chunk_lines.append(
...
with open("structure_quality_report.txt", "w") as f:
    ...
    f.write("=" * 60 + "\n")
    f.write(f"Model 1: Cu Core Only\n")
...
if "cu_c_distances" in locals() and len(c_shell) > 0:
    print(f"  Minimum Cu-C distance: {np.min(cu_c_distances):.3f} Å")

if "c_nn_distances" in locals() and len(c_shell) > 1:

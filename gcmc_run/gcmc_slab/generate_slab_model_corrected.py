...
Ensures both layers span entire XY plane with periodic boundaries
"""
...
# ========== WRITE LAMMPS FILES ==========
def write_lammps_files(atoms, gap_bottom, gap_top, params):
    ...
    cell = atoms.get_cell()
    for i in range(len(positions)):
        positions[i, 0] = positions[i, 0] % cell[0, 0]
        positions[i, 1] = positions[i, 1] % cell[1, 1]

    atoms.set_positions(positions)

    # Write LAMMPS data file
    filename = f"{params.output_dir}/Cu_slab_gap_8.22nm_full.lammps"

    with open(filename, "w") as f:
        # Header
        f.write("LAMMPS data file: Cu slab + porous graphene (FULL XY COVERAGE)\n")
        f.write(f"# Gap: 8.22 nm ({params.gap_size} Å)\n")
        f.write(f"# Box: {cell[0, 0]:.1f} × {cell[1, 1]:.1f} × {cell[2, 2]:.1f} Å\n")
        f.write(
            f"# Pores: {params.pore_diameter} Å diameter, {params.porosity:.1%} porosity\n\n"
        )
        ...
        # Box - triclinic for completeness
    ...
    # Write GCMC region info
    gap_volume = (gap_top - gap_bottom) * params.box_x * params.box_y

    with open(f"{params.output_dir}/gcmc_region.info", "w") as f:
        f.write(f"GCMC_INSERTION_REGION\n")
        f.write(f"region_type = block\n")
        f.write(f"xlo = 0.0\n")
        f.write(f"xhi = {params.box_x}\n")
        f.write(f"ylo = 0.0\n")
        f.write(f"yhi = {params.box_y}\n")
        f.write(f"zlo = {gap_bottom}\n")
        f.write(f"zhi = {gap_top}\n")
        f.write(f"\n# Gap volume: {gap_volume:.0f} Å³\n")

    # Write input script
    # write_input_script(gap_bottom, gap_top, params)
...
def write_input_script(gap_bottom, gap_top, params):
...
# ========== Read Structure ==========
read_data       Cu_slab_gap_8.22nm_full.lammps
...
# ========== Constraints ==========
# Fix bottom Cu atoms
region          cu_bottom block 0 {params.box_x} 0 {params.box_y} 0 10.0
...
variable        mu equal -6.5 + 0.001987*${{T}}*ln(${{P}})

group           CO2 type 3 4

# Enhanced GCMC for gap region
fix             gcmc CO2 gcmc 200 1000 1000 0 12345 ${{T}} ${{mu}} 2.0 &
                mol co2mol &
                region gcmc_region & 
                pressure ${{P}} &
                group CO2 &
...
variable        nCO2_gap equal c_co2_gap/3

# CO2 near Cu surface (within 3.5 Å)
region          cu_surface block 0 {params.box_x} 0 {params.box_y} {gap_bottom} {gap_bottom + 3.5}
compute         co2_cu all count/region cu_surface type 3
variable        nCO2_cu equal c_co2_cu/3

# Gap volume and density
variable        gap_vol equal ({gap_top} - {gap_bottom}) * lx * ly
variable        gap_dens equal v_nCO2_gap * 1660.54 / v_gap_vol  # mmol/cm³

thermo          1000
thermo_style    custom step temp press vol v_nCO2_gap v_nCO2_cu v_gap_dens
...
"""
    ...
    with open(f"{params.output_dir}/run_simulation.lammps", "w") as f:
        f.write(script)

    print(f"✓ LAMMPS input script: {params.output_dir}/run_simulation.lammps")
...
if __name__ == "__main__":
    print("=" * 60)
    print("FULL XY COVERAGE SLAB MODEL - 8.22 nm GAP")
    print("=" * 60)
    ...
    print(f"  Cu lattice: {params.Cu_lattice} Å")
    ...
    # Write files
    write_lammps_files(atoms, gap_bottom, gap_top, params)
    ...
    print("\nFiles created:")
    print("  1. Cu_slab_gap_8.22nm_full.lammps - Structure")
    print("  2. run_simulation.lammps - Input script")
    print("  3. gcmc_region.info - Gap region")
    print("  4. model.xyz - Visualization")

    print("\nTo run simulation:")
    print(f"  cd {params.output_dir}")
    print("  mpirun -np 4 lammps -in run_simulation.lammps")
    print("=" * 60)
